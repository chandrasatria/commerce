	...
	def before_save(self):
		...
		if array_bc["status"] == 0:
			if array_bc["duplicates"]:
				frappe.throw(_("Sorry there are multiple code in batch code <br>"+str(array_bc["duplicates"])))
			else:
				frappe.throw(_("Something wrong :("))
		count = 0
		self.batch_code = ""
...
# !SECTION ---- end of hooks
# SECTION Validate
	def basic_validate(self):
		if not self.voucher_code and self.voucher_title:
			self.voucher_code = self.voucher_title
		if self.voucher_code:
			self.voucher_code = self.voucher_code.upper()
		if self.voucher_claim_type == "Promo Voucher" and not self.voucher_code:
			frappe.throw(_("Please enter some Voucher Code first."))
		# STUB Validate
		if not self.publish_start_date and self.start_date:
			self.publish_start_date = self.start_date
		if not self.publish_end_date and self.end_date:
			self.publish_end_date = self.end_date
		if self.start_date>self.end_date:
			frappe.throw(_("<b>End Date</b> must be greater than <b>Start Date</b>"))
		if self.publish_start_date>self.publish_end_date:
			frappe.throw(_("<b>End Date</b> must be greater than <b>Start Date</b>"))

	def validate_my_voucher(self):
		if self.voucher_claim_type == "My Voucher" and self.quota_type != "Once":
			frappe.throw(_("If you choose <b>My Voucher</b>, then <b>Quota Type</b> must be Once</b>"))
	...
	if len(fga)==0:
		if batch_code:
			...
			if parse["status"] == 1:
				...
				return {"status" : 3, "data":"There are some batch code that duplicates. {}".format(parse["duplicates"])}
		else:
			code_arr = []
		index = 0
...
# Check Rule
	...
	if doc_voucher.voucher_left == 0:
		frappe.throw(_("Voucher telah habis"))
	# for my voucher
	next_code = doc_voucher.next_code
...
def get_my_voucher(voucher,voucher_code):
	mv_exists = frappe.db.exists("My Voucher", {"voucher" : voucher, "voucher_code" : voucher_code})
	if mv_exists:
		doc_my_voucher = frappe.get_doc("My Voucher",mv_exists, "name")
		return doc_my_voucher
	return False

def check_rule_type(voucher,voucher_code=""):
	""" untuk cek voucher type"""
	check_voucher = frappe.db.exists("Voucher", {"name" : voucher})

	if not check_voucher:
		check_voucher = frappe.db.exists("Voucher", {"voucher_code" : voucher})


	if check_voucher:
		doc_voucher = frappe.get_doc("Voucher",check_voucher)
	else:
		return {"status": 0, "keterangan" : "Voucher tidak ditemukan"}

	if doc_voucher.get("voucher_claim_type") == "My Voucher":
		if voucher_code:
			doc_mv = get_my_voucher(voucher,voucher_code)
			if doc_mv:
				if doc_mv.get("status") == "Claimed":
					return {"status": 1, "keterangan" : "ok"}
				else:
					return {"status": 0, "keterangan" : "Voucher telah digunakan/expired."}
			else:
				return {"status" : 0, "keterangan" : "Voucher tidak ditemukan"}
		else:
			return {"status" : 0, "keterangan" : "Voucher tidak ditemukan"}
...
def check_quantity(voucher,customer,for_claim_voucher=0):
	''' jika for_claim_voucher == 1 maka akan cek status akan diabaikan.'''
	status = ("Applied",)
	if for_claim_voucher:
		status = ("Applied","Claimed","Expire")
	...
	if(fga_voucher[0]["quota_type"]=="Once"):
		count = frappe.db.sql("""SELECT COUNT('name') as c FROM `tabMy Voucher` WHERE voucher = %(voucher)s AND status IN %(status)s AND customer = %(customer)s """,{"voucher" : fga_voucher[0]["name"] , "customer" : fga_customer[0]["name"],"status" : status },as_dict=True)

		if(count[0]["c"]>0):
			can_claim = 0
			keterangan = "Sorry, you can only claim once."
	elif(fga_voucher[0]["quota_type"]=="Daily"):
		count = frappe.db.sql("SELECT COUNT('name') as c FROM `tabMy Voucher` WHERE voucher = %(voucher)s AND status IN %(status)s AND customer = %(customer)s AND YEAR(creation) = YEAR(%(today)s) AND MONTH(creation) = MONTH(%(today)s) AND DAY(creation) = DAY(%(today)s)",
		{"voucher" : fga_voucher[0]["name"] , "status": status , "customer":fga_customer[0]["name"], "today":now},as_dict=True)
		if fga_voucher[0]["quota_amount"] != 0:
			if count[0]["c"] >= int(fga_voucher[0]["quota_amount"]):
				can_claim = 0
				keterangan = "Sorry, your daily quota has been run out."
	elif(fga_voucher[0]["quota_type"]=="Monthly"):
		count = frappe.db.sql("""SELECT COUNT('name') as c FROM `tabMy Voucher` WHERE voucher = %(voucher)s AND status IN %(status)s AND customer = %(customer)s AND YEAR(creation) = YEAR(%(today)s) AND MONTH(creation) = MONTH(%(today)s) """,
		{"voucher" : fga_voucher[0]["name"] , "status": status , "customer":fga_customer[0]["name"], "today":now},as_dict=True)
		if fga_voucher[0]["quota_amount"] != 0:
			if count[0]["c"] >= int(fga_voucher[0]["quota_amount"]):
				can_claim = 0
				keterangan = "Sorry, Your monthly quota has been run out."
	if can_claim==0:
		if not keterangan:
			return {"status": 0, "keterangan" : "Maaf, kuota voucher Anda telah habis."}
		else:
...
def check_rule_total_amount(voucher,amount_before_discount):
	now = frappe.utils.now()
	fga_voucher = frappe.get_all("Voucher",fields="min_amt, max_amt", filters={"name":voucher})
	if fga_voucher:
		if fga_voucher[0]["min_amt"] > 0:
			if amount_before_discount < fga_voucher[0]["min_amt"]:
				return {"status": 0, "keterangan" : "Sorry, Voucher minimum purchase is {}".format(frappe.utils.fmt_money(fga_voucher[0]["min_amt"],currency='Rp') )}
		elif fga_voucher[0]["max_amt"] > 0:
			if amount_before_discount > fga_voucher[0]["max_amt"]:
				return {"status": 0, "keterangan" : "Sorry, Voucher maximum purchase is {}".format(frappe.utils.fmt_money(fga_voucher[0]["max_amt"],currency='Rp') )}
		else:
			return {"status" : 1 , "keterangan" : "ok"}
		return {"status" : 1 , "keterangan" : "ok"}
	else:
		return {"status" : 0, "keterangan" : "Voucher not found, please contact Administrator."}
		
def check_voucher_claim_type(voucher, type_voucher):
	now = frappe.utils.now()
	fga_voucher = frappe.get_all("Voucher",fields="voucher_claim_type,name", filters={"name":voucher})
	if fga_voucher:
		if fga_voucher[0]["voucher_claim_type"] == type_voucher:
			return {"status" : 1 , "keterangan" : "ok"}
		else:
			return {"status" : 0 , "keterangan" : "Voucher cannot be claim, voucher type is mismatch."}
	else:
		return {"status" : 0, "keterangan" : "Voucher not found, please contact Administrator."}
...
# # Claim voucher -> My voucher to acquired only
# def claim_voucher_new():
# 	pass

# # Apply Voucher -> Use in Sales Invoice
# @frappe.whitelist(allow_guest=False)
# def apply_voucher(voucher,items=[]):
# 	user = frappe.session.user()
# 	doc_voucher = frappe.get_doc("Voucher", voucher)
# 	customer = frappe.get_list("Customer",fields="*",filters=[["email_id","=",user]])[0]["name"]
# 	doc = frappe.get_doc("Voucher", voucher)
# 	if doc.get("based_on") == "Item":
# 		based_on_item(customer, items=items, voucher_item_based_child= doc_voucher.voucher_item_based_child)
# 	elif doc.get("based_on") == "Item Group":
# 		pass
# 	elif doc.get("based_on") == "Brand":
# 		pass
# 	pass


# def based_on_item(customer,items=[],voucher_item_based_child=[]):
# 	total_qty = 0.0
# 	total_price = 0.0
# 	total_match = 0.0
# 	from api_integration.api_integration.doctype.api_request.api_request import master_get_item_details
# 	for item in items:
# 		item_detail = master_get_item_details(warehouse="", price_list="",customer=customer, company="", transaction_date=frappe.utils.now(),item = item["item_code"])
# 		print(item_detail)
# 		total_price += item_detail["item_detail"]["valuation_rate"]
# 		total_qty += item["qty"]
# 		for v_item in voucher_item_based_child:
# 			if item == v_item["item"]:
# 				total_match += 1

		
# 	return {"total_qty" : total_qty, "total_discount" : 0 , "total_match" :0, "total_price" : 0}
