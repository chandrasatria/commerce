...
def load_notification(self, version = 1):
    ...
    {% set doc_user_integration_settings = frappe.get_doc("User Integration Settings","User Integration Settings") %}
    <center>
                    ...
                    <table style="width: 100%; font-size: 13px; border-spacing: 0px 1px; padding: 0px 10px 0px 7px;">
                        <tr style="background-color: {{doc_application_info.color_primary}}">
                            ...
                            <td style="padding: 10px; font-weight: bold; border-radius: 5px;">
                                <center>
                                    <a style="text-decoration: none; color: white;" href='{{doc_user_integration_settings.reset_password_url}}?key={{doc.key_request}}' target="_blank">
...
def load_html(self,version=1):
    if version == 1:
        ...
        user_integration_settings = frappe.get_single("User Integration Settings")
        if not user_integration_settings.url_frappe:
            frappe.throw(_("Please set up url frappe"))
        if not user_integration_settings.module_name:
            frappe.throw(_("Please set up module name"))
        if not user_integration_settings.login_site:
        ...
        <body>
            ...
            <script>
                var login_site = '"""+user_integration_settings.get("login_site")+ """';
                var using_call_php = """+ str(user_integration_settings.get("using_callphp","0"))+""";
                var base_url = "";
                ...
                if (using_call_php){
                    ...
                    base_url = '"""+user_integration_settings.get("url_frappe")+"""';
                }
             
                var app_name = '"""+user_integration_settings.get("module_name")+"""';
                var imageBanner = "";
